...
int main(int argc, char **argv) {
  ...
  uint64_t sid;
  {
    ...
    sid = (uint64_t)t.tv_sec ^ ((uint64_t)t.tv_nsec << 32) ^ (uint64_t)getpid();
  }
  g_rnd_st = (uint32_t)(sid ^ (sid >> 32));
  if (g_rnd_st == 0) g_rnd_st = 0x6d2b79f5U;
  ...
  for (int i = 0; i < peer_cnt; i++) {
    ...
    for (int j = 0; j < pool.cnt; j++) {
      if (memcmp(pool.re_arr[j].ip, peers[i].ip, 16) == 0 && pool.re_arr[j].port == peers[i].port) {
        is_dup = true;
        break;
      }
    }
  ...
  if (stdin_flg >= 0) fcntl(STDIN_FILENO, F_SETFL, stdin_flg | O_NONBLOCK);
  ev.events = EPOLLIN;
  ev.data.u64 = ID_STD;
  ...
  if (slash) {
    ...
    if (dlen >= sizeof(cfg_dir)) dlen = sizeof(cfg_dir) - 1;
  ...
  if (cfg_ifd < 0) {
    ...
    int wd = inotify_add_watch(cfg_ifd, cfg_dir, IN_CLOSE_WRITE | IN_MOVED_TO);
    if (wd < 0) {
  ...
  fflush(stdout);
  on_tmr(timer_fd, &udp, &cry_ctx, &rt, &cfg, &gsp_off, cfg_path, act_port, sid, &pool);
        ...
        if (cfg_ifd >= 0) {
          ...
          if (n > 0) {
            ...
            for (ssize_t off = 0; off < n;) {
              struct inotify_event *ie = (struct inotify_event *)(evbuf + off);
              if (ie->len > 0 && strncmp(ie->name, cfg_file, sizeof(cfg_file)) == 0) {
                if ((ie->mask & IN_CLOSE_WRITE) || (ie->mask & IN_MOVED_TO)) need_reload = true;
              }
              off += (ssize_t)sizeof(struct inotify_event) + (ssize_t)ie->len;
            }
            if (need_reload) {
              cfg_reload_apply(&cfg, &cry_ctx, &rt, &pool, cfg_path, sys_ts());
            }
          }
        }
      }
