...
static bool tap_tx_use_tcp (const Rt *rt, const Cfg *cfg,
                            const uint8_t ip[16], uint16_t port);
static void *buf_alloc_align (size_t align, size_t size);
typedef struct LoopRxCtx
{
  int tap_fd;
  Udp *udp;
  Cry *cry_ctx;
               ...
               uint64_t sid)
{
  ...
  tap_pipe->sid = sid;
  tap_pipe->q_arr = calloc (BATCH_MAX, sizeof (*tap_pipe->q_arr));
  if (!tap_pipe->q_arr)
    {
      tap_pipe_free (tap_pipe);
      return -1;
    }
  atomic_init (&tap_pipe->stop_req, false);
...
tap_pipe_sync (TapPipe *tap_pipe, const Rt *rt, const Cfg *cfg, uint64_t now)
{
  ...
  if (rt_cpy (&tap_pipe->rt, rt) == 0)
    {
      Cfg new_cfg;
      if (cfg_cpy (&new_cfg, cfg) == 0)
        {
          cfg_free (&tap_pipe->cfg);
          tap_pipe->cfg = new_cfg;
          tap_pipe->snap_ts = now;
        }
      else
        {
          fprintf (stderr, "main: tap snapshot config refresh failed\n");
        }
    }
  else
    {
...
tap_pipe_start (TapPipe *tap_pipe)
{
  ...
  if (pthread_create (&tap_pipe->tid_arr[1], NULL, tap_loop, tap_pipe)
      != 0)
    {
...
tap_pipe_stop (TapPipe *tap_pipe)
{
  if (!tap_pipe)
    return;
  atomic_store_explicit (&tap_pipe->stop_req, true, memory_order_release);
...
tap_loop (void *arg)
{
  ...
  for (;;)
    {
      pthread_mutex_lock (&tap_pipe->q_mtx);
      while (!atomic_load_explicit (&tap_pipe->stop_req, memory_order_acquire)
             && tap_pipe->q_cnt == 0)
  ...
  for (uint32_t i = 0; i < rt->cnt; i++)
    {
      ...
      if (memcmp (re->lla, lla, 16) != 0)
        continue;
      if (re->state == RT_DED || re->ep_port == 0)
        continue;
...
static bool
rt_static_peer_has (const Rt *rt, const uint8_t peer_lla[16],
                    const uint8_t src_ip[16], uint16_t src_port)
{
  ...
  for (uint32_t i = 0; i < rt->cnt; i++)
    {
      const Re *re = &rt->re_arr[i];
      if (re->r2d != 0 || !re->is_static)
        continue;
      if (peer_lla && IS_LLA_VAL (peer_lla)
          && memcmp (re->lla, peer_lla, 16) == 0)
        return true;
      if (!src_ip || src_port == 0)
        continue;
      if (memcmp (re->ep_ip, src_ip, 16) == 0 && re->ep_port == src_port)
        return true;
    }
  return false;
}
...
static bool
p2p_direct_peer_ok (const Cfg *cfg, const Rt *rt, const TpSrc *tp_src,
                    const uint8_t src_ip[16], uint16_t src_port)
{
  if (!cfg)
    return false;
  if (cfg->p2p == P2P_EN)
    return true;
  return rt_static_peer_has (rt, tp_src ? tp_src->peer_lla : NULL, src_ip,
                             src_port);
}

static bool
ctrl_send (Udp *udp, const Rt *rt, const Cfg *cfg, const TpSrc *tp_src,
           const uint8_t dst_ip[16], uint16_t dst_port, const uint8_t *buf,
           size_t len)
{
  if (tp_src && tp_src->is_tcp && tp_src->tcp_fd >= 0)
    return tp_send_fd (tp_src->tcp_fd, buf, len);
  if (!udp || !rt || !cfg || !dst_ip || dst_port == 0 || !buf || len == 0)
    return false;
  return tp_send_ctrl (udp, rt, cfg, dst_ip, dst_port, buf, len);
}

static void
ctrl_tx_mark (Rt *rt, uint64_t *cnt, size_t len, const uint8_t dst_ip[16],
              uint16_t dst_port, uint64_t ts)
{
  if (!rt)
    return;
  if (cnt)
    (*cnt)++;
  rt->ctrl_tx_b += (uint64_t)len;
  if (dst_ip && dst_port != 0)
    rt_tx_ack (rt, dst_ip, dst_port, ts);
}
          ...
          uint64_t sid, bool may_rel)
{
  if (!udp || !cry_ctx || !rt || !cfg || !re)
    return;
  bool use_tcp = tap_tx_use_tcp (rt, cfg, re->ep_ip, re->ep_port);
  if (!lla_is_z (re->lla) && may_rel && cfg->p2p == P2P_EN)
    {
      RtDec rd = rt_sel (rt, re->lla, true);
      if (rd.type == RT_REL
          && !tap_tx_use_tcp (rt, cfg, rd.rel.relay_ip, rd.rel.relay_port))
        {
          ...
          if (tp_send_ctrl (udp, rt, cfg, rd.rel.relay_ip, rd.rel.relay_port,
                            hp_buf, hp_len))
            {
              ctrl_tx_mark (rt, &rt->hp_tx_cnt, hp_len, rd.rel.relay_ip,
                            rd.rel.relay_port, ts);
              re->hp_ts = ts;
            }
        }
    }
  if (!use_tcp && re->r2d == 0 && re->is_static && re->ep_port != 0)
    (void)tp_probe (rt, cfg, re->ep_ip, re->ep_port);
  if (use_tcp)
    {
      (void)sid;
      (void)tp_probe (rt, cfg, re->ep_ip, re->ep_port);
...
static void
rt_fmt_mtu (char *buf, size_t cap, const Rt *rt, const RtDec *sel, bool use_tcp)
{
  if (!buf || cap == 0)
    return;
  if (!rt || !sel || use_tcp)
  ...
  if (is_srch)
    {
      if (prb_mtu > 0)
        snprintf (buf, cap, "%u (%u-%u, %u)", pmtu, lkg, ukb, prb_mtu);
      else
        snprintf (buf, cap, "%u (%u-%u)", pmtu, lkg, ukb);
      return;
    }
  if (mtu_st == MTU_ST_F || is_fixed)
    snprintf (buf, cap, "%u", pmtu);
  else
    snprintf (buf, cap, "%u (LKG:%u, UKB:%u, base)", pmtu, lkg, ukb);
}
...
static uint32_t
status_row_rtt (const RRow *row)
{
  if (!row || row->metric[0] == '\0' || strcmp (row->metric, "-") == 0)
    return RT_M_INF;
  char *end = NULL;
  unsigned long v = strtoul (row->metric, &end, 10);
  if (end == row->metric)
    return RT_M_INF;
  return v >= RT_M_INF ? RT_M_INF : (uint32_t)v;
}

static int
status_row_cmp (const void *a, const void *b)
{
  const RRow *ra = (const RRow *)a;
  const RRow *rb = (const RRow *)b;
  uint32_t ma = status_row_rtt (ra);
  uint32_t mb = status_row_rtt (rb);
  if (ma != mb)
    return (ma < mb) ? -1 : 1;
  int nh_cmp = strcmp (ra->nh, rb->nh);
  if (nh_cmp != 0)
    return nh_cmp;
  int st_cmp = strcmp (ra->st, rb->st);
  if (st_cmp != 0)
    return st_cmp;
  int mtu_cmp = strcmp (ra->mtu, rb->mtu);
  if (mtu_cmp != 0)
    return mtu_cmp;
  return strcmp (ra->loss, rb->loss);
}
                  ...
                  const char *cfg_path, uint64_t ts)
{
  ...
  if (cfg->port != new_cfg.port || cfg->l_exp != new_cfg.l_exp)
      fprintf (stderr,
             "main: port changed in config but not hot-applied\n");
  ...
  if (cfg->tp_mask != new_cfg.tp_mask
      || cfg->tp_pref_len != new_cfg.tp_pref_len
      || (cfg->tp_pref_len > 0
          && memcmp (cfg->tp_pref, new_cfg.tp_pref, cfg->tp_pref_len) != 0))
    {
      free (cfg->tp_pref);
      cfg->tp_mask = new_cfg.tp_mask;
      cfg->tp_pref_len = new_cfg.tp_pref_len;
      cfg->tp_pref_cap = new_cfg.tp_pref_cap;
      cfg->tp_pref = new_cfg.tp_pref;
      new_cfg.tp_pref = NULL;
      new_cfg.tp_pref_cap = 0;
      new_cfg.tp_pref_len = 0;
      fprintf (stderr, "main: reloaded transport mode\n");
    }
  if (cfg->bogon_cnt != new_cfg.bogon_cnt
      || (cfg->bogon_cnt > 0
          && memcmp (cfg->bogon_arr, new_cfg.bogon_arr,
                     sizeof (*cfg->bogon_arr) * cfg->bogon_cnt)
                 != 0))
    {
      free (cfg->bogon_arr);
      cfg->bogon_cnt = new_cfg.bogon_cnt;
      cfg->bogon_cap = new_cfg.bogon_cap;
      cfg->bogon_arr = new_cfg.bogon_arr;
      new_cfg.bogon_arr = NULL;
      new_cfg.bogon_cap = 0;
      new_cfg.bogon_cnt = 0;
      bogon_cfg_apply (cfg);
  ...
  P *peers = NULL;
  int peer_cnt = 0;
  if (p_arr_load (cfg_path, &peers, &peer_cnt) != 0)
    {
      fprintf (stderr, "main: failed to reload peer list from config\n");
      cfg_free (&new_cfg);
      return;
    }
  int add_cnt = 0;
  for (int i = 0; i < peer_cnt; i++)
    {
      ...
      if (!pool_has_peer (pool, peers[i].ip, peers[i].port))
        {
          int old_cnt = pool->cnt;
          pp_add (pool, peers[i].ip, peers[i].port);
          if (pool->cnt > old_cnt)
            add_cnt++;
        }
    }
          ...
          bool daemon_child, LoopReadyFn ready_fn, void *ready_arg)
{
  ...
  bool tap_inline = false;
  switch (cfg.tap_mode)
    {
    case TAP_MODE_INLINE:
      tap_inline = true;
      break;
    case TAP_MODE_PIPE:
      tap_inline = false;
    ...
    case TAP_MODE_AUTO:
    default:
      tap_inline = (cpu_aff_avail_cnt () <= 1);
      break;
    }
  int epfd = -1;
  int timer_fd = -1;
  ...
  if (p_arr_load (cfg_path, &peers, &peer_cnt) != 0)
    {
      fprintf (stderr, "main: failed to load static peers from config\n");
      goto out;
    }
  ...
  if (tp_rt_init (&tp_rt, act_port) != 0)
    {
      fprintf (stderr, "main: failed to bind tcp port %u\n", act_port);
      goto out;
    }
  tp_inited = true;
  ...
  ev.events = EPOLLIN;
  ev.data.u64 = TP_EV_LISTEN;
  if (epoll_ctl (epfd, EPOLL_CTL_ADD, tp_rt_listen_fd_get (&tp_rt), &ev) != 0)
    {
      perror ("main: epoll add tcp listener failed");
      goto out;
    }
  tp_glb_bind (&tp_rt, epfd, &cry_ctx, &cfg, sid);
  ...
  if (ready_fn)
    ready_fn (ready_arg);
  rc = 0;
  LoopRxCtx tcp_rx_ctx = { .tap_fd = tap_fd,
                           .udp = &udp,
                           .cry_ctx = &cry_ctx,
                           .rt = &rt,
                           .cfg = &cfg,
...
typedef struct
{
  ...
  int seg_bgn;
  int seg_end;
  uint8_t (*buf_arr)[TAP_F_MAX + TAP_HR + TAP_TR];
...
typedef struct
{
  ...
  uint8_t (*flood_lla)[16];
  size_t flood_lla_cap;
  TapFloodEp *flood_ep;
  size_t flood_ep_cap;
  uint8_t (*frag_bufs)[UDP_PL_MAX + TAP_HR];
  uint8_t (*uso_bufs)[TAP_F_MAX + TAP_HR + TAP_TR];
  UdpMsg *batch_arr;
  int bc;
} TapRun;

static _Thread_local TapRun *g_tap_run;
static _Thread_local uint8_t (*g_tap_frame_bufs)[TAP_F_MAX + TAP_HR + TAP_TR];

static void *
buf_alloc_align (size_t align, size_t size)
{
  void *ptr = NULL;
  if (size == 0)
    return NULL;
...
static bool
loop_arr_rsv (void **arr, size_t *cap, size_t need, size_t elem_sz)
{
  if (!arr || !cap)
    return false;
  if (need <= *cap)
  ...
  if (elem_sz == 0 || new_cap > (SIZE_MAX / elem_sz))
    return false;
  void *new_arr = realloc (*arr, elem_sz * new_cap);
  if (!new_arr)
    return false;
  *arr = new_arr;
  *cap = new_cap;
...
tap_run_get (void)
{
  if (g_tap_run)
    return g_tap_run;
  TapRun *tap_run = calloc (1, sizeof (*tap_run));
  if (!tap_run)
    return NULL;
  tap_run->frag_bufs
      = buf_alloc_align (32, sizeof (*tap_run->frag_bufs) * BATCH_MAX);
  tap_run->uso_bufs = buf_alloc_align (32,
                                       sizeof (*tap_run->uso_bufs)
                                           * TAP_TSO_PAR_MAX);
  tap_run->batch_arr = calloc (BATCH_MAX, sizeof (*tap_run->batch_arr));
  if (!tap_run->frag_bufs || !tap_run->uso_bufs || !tap_run->batch_arr)
    {
      free (tap_run->batch_arr);
      free (tap_run->uso_bufs);
      free (tap_run->frag_bufs);
      free (tap_run);
      return NULL;
    }
  g_tap_run = tap_run;
...
static uint8_t (*
tap_frame_bufs_get (void))[TAP_F_MAX + TAP_HR + TAP_TR]
{
  if (g_tap_frame_bufs)
    return g_tap_frame_bufs;
  g_tap_frame_bufs
      = buf_alloc_align (32, sizeof (*g_tap_frame_bufs) * BATCH_MAX);
  return g_tap_frame_bufs;
}
...
tap_tso_par_boot (void)
{
  ...
  g_tso_par.buf_arr
      = buf_alloc_align (32, sizeof (*g_tso_par.buf_arr) * TAP_TSO_PAR_MAX);
  if (!g_tso_par.buf_arr)
    return;
...
fail_buf:
  free (g_tso_par.buf_arr);
  g_tso_par.buf_arr = NULL;
}
                       ...
                       uint8_t *vnet_frame, size_t vnet_len, UdpMsg *msg)
{
  if (!cry_ctx || !tx_path || !tx_path->is_val || tx_path->type == RT_NONE
      || tx_path->use_tcp || !vnet_frame || !msg)
    return false;
  size_t out_len = 0;
                ...
                uint16_t port)
{
  if (!cfg || !ip || port == 0)
    return false;
  return cfg_tp_pick (cfg, rt_ep_tp_mask (rt, ip, port)) == TP_PROTO_TCP;
}
               ...
               uint64_t sid, const TapTxPath *tx_path)
{
  ...
  if (cfg->p2p != P2P_EN || tx_path->type != RT_REL)
    return;
  Re *hp_re = rt_re_fnd (rt, tx_path->rt_key);
  if (hp_re)
    pulse_tx (udp, cry_ctx, rt, cfg, hp_re, now, sid, true);
  for (uint32_t i = 0; i < rt->cnt; i++)
    {
      Re *re = &rt->re_arr[i];
      if (re == hp_re)
      ...
      if (re->r2d != 0)
        continue;
      if (memcmp (re->lla, tx_path->rt_key, 16) != 0)
        continue;
      if (re->state == RT_DED || re->ep_port == 0)
        continue;
      pulse_tx (udp, cry_ctx, rt, cfg, re, now, sid, false);
    }
}
             ...
             bool tx_ack_apply, bool mss_apply)
{
  ...
  if ((dst_mac[0] & 0x01U) != 0U)
    {
      ...
      if (!loop_arr_rsv ((void **)&tap_run->flood_lla, &tap_run->flood_lla_cap,
                         flood_need, sizeof (*tap_run->flood_lla))
          || !loop_arr_rsv ((void **)&tap_run->flood_ep, &tap_run->flood_ep_cap,
                            flood_need, sizeof (*tap_run->flood_ep)))
        return false;
      uint8_t (*uniq_lla)[16] = tap_run->flood_lla;
      int u_cnt = 0;
      for (uint32_t ri = 0; ri < rt->cnt; ri++)
        {
          Re *re = &rt->re_arr[ri];
          if (!re->is_act || re->state == RT_DED)
            continue;
      ...
      TapFloodEp *ep_arr = tap_run->flood_ep;
      int ep_cnt = 0;
      for (int u = 0; u < u_cnt; u++)
        {
          ...
          if (sel.type == RT_DIR)
          ...
          else if (sel.type == RT_VP)
            {
              uint8_t gw_ip[16];
              uint16_t gw_port = 0;
              if (rt_gw_fnd (rt, cfg->addr, cfg->p2p != P2P_EN, gw_ip,
                             &gw_port))
                {
  ...
  if (!is_tiny && __builtin_expect ((size_t)vnet_len <= (size_t)max_vnet, 1))
  ...
  else
    {
      ...
      while (off < vnet_len)
        {
          if (!tx_path_lcl.use_tcp && *bc + 1 > BATCH_MAX
              && !tap_batch_fls (udp, batch_arr, bc,
  ...
  if (tap_tso_fit (vnet_frame, vnet_len, &tso))
    {
      ...
      while (tap_tso_seg (&tso, tap_run->uso_bufs[tso_i] + TAP_HR, &tso_len))
        {
          if (has_tx_path && !tx_path.use_tcp && max_vnet > 0
              && tso_len <= max_vnet)
            {
              if (!tap_data_fast (udp, cry_ctx, &tx_path, now,
...
{
  uint8_t (*frame_bufs)[TAP_F_MAX + TAP_HR + TAP_TR] = tap_frame_bufs_get ();
  if (!frame_bufs)
    return;
  for (;;)
    {
...
static bool
rx_src_ep_get (const TpSrc *tp_src, const uint8_t udp_src_ip[16],
               uint16_t udp_src_port, uint8_t out_ip[16], uint16_t *out_port)
{
  if (!out_ip || !out_port)
    return false;
  memset (out_ip, 0, 16);
  *out_port = 0;
  if (tp_src && tp_src->route_port != 0)
    {
      memcpy (out_ip, tp_src->route_ip, 16);
      *out_port = tp_src->route_port;
      return true;
    }
  if (udp_src_ip && udp_src_port != 0)
    {
      memcpy (out_ip, udp_src_ip, 16);
      *out_port = udp_src_port;
      return true;
    }
  return false;
}
               ...
               uint8_t *pt, size_t pt_len, uint64_t ts)
{
  ...
  int tap_fd = ctx->tap_fd;
  Udp *udp = ctx->udp;
  Cry *cry_ctx = ctx->cry_ctx;
  Rt *rt = ctx->rt;
  const Cfg *cfg = ctx->cfg;
  uint64_t sid = ctx->sid;
  PPool *pool = ctx->pool;
  ...
  switch (hdr->pkt_type)
    ...
    case PT_FRAG:
      {
        ...
        if (payload_len < sizeof (FragHdr))
          break;
        uint32_t msg_id = ((uint32_t)payload[0] << 24)
        ...
        if (hdr->rel_f != 0 && memcmp (dest_lla, cfg->addr, 16) != 0)
          {
            if (has_src)
              relay_fwd_frag (udp, cry_ctx, rt, cfg, dest_lla, msg_id, frag_off,
                              mf, chunk, chunk_len,
                              (uint8_t)(hdr->hop_c - 1), ts, src_ip, src_port);
            break;
          }
        ...
        uint16_t full_len = 0;
        uint8_t *full_l3
            = frag_asm (src_ip, msg_id, frag_off, mf, chunk, chunk_len,
                        &full_len);
        ...
        for (uint32_t ack = 0; ack < RT_PRB_BST; ack++)
          {
            if (!ctrl_send (udp, rt, cfg, tp_src, src_ip, src_port, ack_buf,
                            ack_len))
              break;
            ctrl_tx_mark (rt, NULL, ack_len, src_ip, src_port, ts);
          }
        break;
      }
    case PT_MTU_ACK:
    ...
    case PT_STAT_REQ:
      {
        if (!has_src || !is_ip_loopback (src_ip))
          break;
        uint32_t req_id = 0;
    ...
    case PT_PING:
      {
        ...
        if (ctrl_send (udp, rt, cfg, tp_src, has_src ? src_ip : NULL, src_port,
          ...
          ctrl_tx_mark (rt, &rt->pong_tx_cnt, pong_len,
                        has_src ? src_ip : NULL, src_port, ts);
        break;
      }
    case PT_PONG:
      {
    ...
    case PT_SEQ_REQ:
      {
        ...
        if (tp_src && tp_src->is_tcp && IS_LLA_VAL (tp_src->peer_lla))
          {
            memcpy (peer_lla, tp_src->peer_lla, 16);
            peer_known = true;
          }
        else
          {
            peer_known = rt_ep_peer_lla (rt, src_ip, src_port, peer_lla);
          }
        if (peer_known)
        ...
        if (dt_len > 0
            && ctrl_send (udp, rt, cfg, tp_src, src_ip, src_port, dt_buf,
                          dt_len))
          ctrl_tx_mark (rt, &rt->gsp_dt_tx_cnt, dt_len, src_ip, src_port, ts);
        break;
      }
    case PT_HP:
      {
        if (cfg->p2p == P2P_EN)
          on_hp (pt, pt_len, cry_ctx, udp, rt, cfg, cfg->addr, sid, ts);
        break;
      }
    }
}
...
ctrl_rate_upd (Rt *rt, uint64_t ts)
{
  ...
  rt->ctrl_now_rx_bps
      = ((rt->ctrl_rx_b - rt->ctrl_last_rx_b) * 1000ULL) / diff_ms;
  rt->ctrl_last_ts = ts;
  rt->ctrl_last_tx_b = rt->ctrl_tx_b;
  rt->ctrl_last_rx_b = rt->ctrl_rx_b;
}
        ...
        uint16_t act_port, uint64_t sid, PPool *pool)
{
  ...
  if (rt->loc_last_ts == 0 || ts < rt->loc_last_ts
    ...
    {
      rt_loc_add (rt, cfg->addr, act_port, cfg->tp_mask, ts);
      rt->loc_last_ts = ts;
    }
  rt_prn_st (rt, cfg, ts);
  rt_src_gc (rt, ts);
  ...
  for (uint32_t i = 0; i < rt->cnt; i++)
    {
      ...
      if (re->r2d != 0)
        continue;
      if (cfg->p2p != P2P_EN && !re->is_static)
        continue;
...
#define STATUS_PUT(...)                                                        \
  ...
  uint64_t up_ms = (rt->boot_ts > 0 && now > rt->boot_ts) ? (now - rt->boot_ts)
                                                           : 0;
  uint64_t ctrl_avg_tx_bps
      = (up_ms > 0) ? ((rt->ctrl_tx_b * 1000ULL) / up_ms) : rt->ctrl_tx_b;
  uint64_t ctrl_avg_rx_bps
      = (up_ms > 0) ? ((rt->ctrl_rx_b * 1000ULL) / up_ms) : rt->ctrl_rx_b;
  char ctrl_avg_tx_str[32];
  char ctrl_avg_rx_str[32];
  char ctrl_now_tx_str[32];
  char ctrl_now_rx_str[32];
  char ctrl_tot_str[32];
  char ctrl_tx_str[32];
  char ctrl_rx_str[32];
  fmt_rate (ctrl_avg_tx_str, sizeof (ctrl_avg_tx_str), ctrl_avg_tx_bps);
  fmt_rate (ctrl_avg_rx_str, sizeof (ctrl_avg_rx_str), ctrl_avg_rx_bps);
  fmt_rate (ctrl_now_tx_str, sizeof (ctrl_now_tx_str), rt->ctrl_now_tx_bps);
  fmt_rate (ctrl_now_rx_str, sizeof (ctrl_now_rx_str), rt->ctrl_now_rx_bps);
  ...
  UnmappedEp *um_arr = NULL;
  size_t um_cap = 0;
  size_t base_need = (rt->cnt > 0) ? (size_t)rt->cnt : 1U;
  if (!loop_arr_rsv ((void **)&um_arr, &um_cap, base_need, sizeof (*um_arr)))
    {
      STATUS_PUT ("status: out of memory while building route tables\n");
      len = (len < cap) ? len : cap;
      return len;
    }
  ...
  RRow *rows = NULL;
  size_t rows_cap = 0;
  size_t row_need = (rt->cnt > (UINT32_MAX / 3U)) ? (size_t)rt->cnt
                                                  : (size_t)rt->cnt * 3U;
  if (row_need == 0)
    row_need = 1U;
  if (!loop_arr_rsv ((void **)&uniq_lla, &uniq_cap, base_need,
                        sizeof (*uniq_lla))
      || !loop_arr_rsv ((void **)&rows, &rows_cap, row_need, sizeof (*rows)))
    {
      free (rows);
      free (uniq_lla);
      free (um_arr);
  ...
  for (int u = 0; u < u_cnt; u++)
    {
      ...
      if (sel.type == RT_NONE || sel.type == RT_VP)
      ...
      else
        {
          ...
          if (has_loss)
            ...
            strcpy (row->loss, "-");
          rt_fmt_mtu (row->mtu, sizeof (row->mtu), rt, &sel, use_tcp);
      ...
      int cand_start = r_cnt;
      for (uint32_t i = 0; i < rt->cnt; i++)
        {
          const Re *re = &rt->re_arr[i];
          if (re->state == RT_DED)
            continue;
          if (memcmp (re->lla, uniq_lla[u], 16) != 0)
            continue;
          bool is_sel = false;
          if ((sel.type == RT_DIR || sel.type == RT_REL)
              && memcmp (re->ep_ip, sel_ip, 16) == 0
              && re->ep_port == sel_port)
            {
              continue;
            }
          if (sel.type == RT_DIR && re->r2d == 0
              && memcmp (re->ep_ip, sel.dir.ip, 16) == 0
              && re->ep_port == sel.dir.port)
            {
              is_sel = true;
            }
          else if (sel.type == RT_REL && re->r2d != 0
                   && memcmp (re->ep_ip, sel.rel.relay_ip, 16) == 0
                   && re->ep_port == sel.rel.relay_port)
            {
              is_sel = true;
            }
          ...
          row->dst[0] = '\0';
          RtDec alt_sel;
          memset (&alt_sel, 0, sizeof (alt_sel));
          if (re->r2d == 0)
            {
              alt_sel.type = RT_DIR;
              memcpy (alt_sel.dir.ip, re->ep_ip, 16);
              alt_sel.dir.port = re->ep_port;
            }
          else
            {
              alt_sel.type = RT_REL;
              memcpy (alt_sel.rel.relay_ip, re->ep_ip, 16);
              alt_sel.rel.relay_port = re->ep_port;
              memcpy (alt_sel.rel.relay_lla, re->nhop_lla, 16);
            }
          bool use_tcp_alt = tap_tx_use_tcp (rt, cfg, re->ep_ip, re->ep_port);
          uint32_t alt_rtt
              = rt_show_rtt (rt, uniq_lla[u], &alt_sel, use_tcp_alt);
          uint32_t alt_loss = 0;
          bool has_alt_loss
              = !use_tcp_alt
                && rt_show_loss (rt, uniq_lla[u], &alt_sel, &alt_loss);
          char ip_str[INET6_ADDRSTRLEN];
          if (re->ep_ip[0] == 0 && re->ep_ip[1] == 0 && re->ep_ip[10] == 0xff
              && re->ep_ip[11] == 0xff)
            inet_ntop (AF_INET, re->ep_ip + 12, ip_str, sizeof (ip_str));
          else
            inet_ntop (AF_INET6, re->ep_ip, ip_str, sizeof (ip_str));
          char ep_alt[64];
          snprintf (ep_alt, sizeof (ep_alt), "%s:%u", ip_str, re->ep_port);
          char nh_alt[INET6_ADDRSTRLEN] = "-";
          if (re->r2d == 0)
            inet_ntop (AF_INET6, uniq_lla[u], nh_alt, sizeof (nh_alt));
          else if (IS_LLA_VAL (re->nhop_lla))
            inet_ntop (AF_INET6, re->nhop_lla, nh_alt, sizeof (nh_alt));
          snprintf (row->nh, sizeof (row->nh), "%s (%s)", nh_alt, ep_alt);
          if (re->is_act && re->state == RT_ACT)
            strcpy (row->st, "active");
          else if (re->state == RT_PND)
            strcpy (row->st, "pending");
          else
            strcpy (row->st, "via");
          rt_fmt_mtu (row->mtu, sizeof (row->mtu), rt, &alt_sel,
                      use_tcp_alt);
          if (alt_rtt >= RT_M_INF)
            strcpy (row->metric, "-");
          else
            snprintf (row->metric, sizeof (row->metric), "%ums", alt_rtt);
          if (has_alt_loss)
            snprintf (row->loss, sizeof (row->loss), "%u%%", alt_loss);
          else
            strcpy (row->loss, "-");
          if ((int)strlen (row->dst) > m_dst)
            m_dst = (int)strlen (row->dst);
      ...
      if ((r_cnt - cand_start) > 1)
        qsort (rows + cand_start, (size_t)(r_cnt - cand_start),
               sizeof (*rows), status_row_cmp);
    }
  ...
  STATUS_PUT ("ctrl: gossip_tx=%llu gsp_dt_tx=%llu ping_tx=%llu pong_tx=%llu hp_tx=%llu seqreq_tx=%llu now_tx=%s now_rx=%s avg_tx=%s avg_rx=%s total=%s tx=%s rx=%s\n",
              (unsigned long long)rt->gsp_tx_cnt,
              (unsigned long long)rt->gsp_dt_tx_cnt,
              (unsigned long long)rt->ping_tx_cnt,
              (unsigned long long)rt->pong_tx_cnt,
              (unsigned long long)rt->hp_tx_cnt,
              (unsigned long long)rt->seqreq_tx_cnt, ctrl_now_tx_str,
              ctrl_now_rx_str, ctrl_avg_tx_str, ctrl_avg_rx_str,
