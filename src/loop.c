...
void on_udp_emsg(const uint8_t dst_ip[16], uint16_t dst_port, size_t atmpt_plen) {
  if (!g_rt) return;
...
static bool p_is_me(const Rt *rt, const uint8_t our_lla[16], const uint8_t ip[16], uint16_t port) {
  for (uint32_t i = 0; i < rt->cnt; i++) {
    const Re *re = &rt->re_arr[i];
    if (memcmp(re->lla, our_lla, 16) != 0) continue;
    if (re->r2d != 0) continue;
    if (memcmp(re->ep_ip, ip, 16) != 0) continue;
    if (re->ep_port != port) continue;
    return true;
  }
  return false;
}
...
void tty_raw(void) {
  ...
  struct termios t;
  if (tcgetattr(STDIN_FILENO, &g_tty_old) != 0) return;
  g_tty_sav = true;
  atexit(tty_rst);
  t = g_tty_old;
...
static uint32_t rt_e2e_m(const Rt *rt, const uint8_t dst_lla[16], const RtDec *sel) {
  if (sel->type == RT_DIR) {
    uint32_t best = RT_M_INF;
    for (uint32_t i = 0; i < rt->cnt; i++) {
      const Re *re = &rt->re_arr[i];
      if (!re->is_act || re->state == RT_DED) continue;
      if (re->r2d != 0) continue;
      if (memcmp(re->lla, dst_lla, 16) != 0) continue;
      if (memcmp(re->ep_ip, sel->dir.ip, 16) != 0) continue;
      if (re->ep_port != sel->dir.port) continue;
      uint32_t m = re_m(re);
      if (m < best) best = m;
    }
    return best;
  ...
  if (sel->type == RT_REL) {
    ...
    for (uint32_t i = 0; i < rt->cnt; i++) {
      ...
      if (re->r2d == 0) continue;
      if (memcmp(re->lla, dst_lla, 16) != 0) continue;
      if (memcmp(re->ep_ip, sel->rel.relay_ip, 16) != 0) continue;
      if (re->ep_port != sel->rel.relay_port) continue;
      uint32_t m_adv = (re->r2d > 0) ? re->r2d : re->adv_m;
      if (m_adv == 0 || m_adv == RTT_UNK || m_adv >= RT_M_INF) m_adv = re->rt_m;
      if (m_adv == 0 || m_adv == RTT_UNK || m_adv >= RT_M_INF) continue;
      if (m_adv < m_rd) m_rd = m_adv;
    }
    ...
    if (m_rd >= RT_M_INF || m_lr >= RT_M_INF) return RT_M_INF;
    return m_lr + m_rd;
  }
  return RT_M_INF;
}

static bool rt_key_get(const Rt *rt, const uint8_t *frame, const uint8_t our_lla[16], uint8_t out_lla[16]) {
  (void)rt;
  if (!frame || !out_lla || !our_lla) return false;
  const uint8_t *dst_mac = frame;
  if (dst_mac[0] != 0x02 || dst_mac[1] != 0x00) return false;
...
static bool mesh_dst_lla_from_frame(const uint8_t *frame, const uint8_t base_lla[16], uint8_t out_lla[16]) {
  if (!frame || !base_lla || !out_lla) return false;
  const uint8_t *dst_mac = frame;
  if (dst_mac[0] != 0x02 || dst_mac[1] != 0x00) return false;
...
void rt_loc_add(Rt *rt, const uint8_t our_lla[16], uint16_t port, uint64_t now) {
...
static bool pool_has_peer(const PPool *pool, const uint8_t ip[16], uint16_t port) {
  for (int i = 0; i < pool->cnt; i++) {
    if (memcmp(pool->re_arr[i].ip, ip, 16) == 0 && pool->re_arr[i].port == port) return true;
  }
  return false;
}

void cfg_reload_apply(Cfg *cfg, Cry *cry_ctx, Rt *rt, PPool *pool, const char *cfg_path, uint64_t ts) {
  Cfg new_cfg;
  if (cfg_load(cfg_path, &new_cfg) != 0) return;
  ...
  if (cfg->port != new_cfg.port || cfg->l_exp != new_cfg.l_exp)
    fprintf(stderr, "main: listen/port changed in config but not hot-applied\n");
  ...
  for (int i = 0; i < peer_cnt; i++) {
    ...
    ne.rto = RTO_INIT;
    rt_upd(rt, &ne, ts);
    if (!pool_has_peer(pool, peers[i].ip, peers[i].port) && pool->cnt < PEER_MAX) {
...
void on_tap(int tap_fd, Udp *udp, Cry *cry_ctx, Rt *rt, const Cfg *cfg, uint64_t sid, uint64_t now) {
  (void)sid;
  ...
  for (int i = 0; i < BATCH_MAX; i++) {
    ...
    if ((dst_mac[0] & 0x01U) != 0U) {
      ...
      for (uint32_t ri = 0; ri < rt->cnt && u_cnt < 256; ri++) {
        Re *re = &rt->re_arr[ri];
        if (!re->is_act || re->state == RT_DED) continue;
        if (memcmp(re->lla, z_lla, 16) == 0) continue;
        if (memcmp(re->lla, cfg->addr, 16) == 0) continue;
        bool is_dup = false;
        ...
        if (!is_dup) memcpy(uniq_lla[u_cnt++], re->lla, 16);
      }
      typedef struct { uint8_t ip[16]; uint16_t port; } FloodEp;
      FloodEp ep_arr[256];
      int ep_cnt = 0;
      for (int u = 0; u < u_cnt && ep_cnt < 256; u++) {
        ...
        if (!is_ok || ep_port == 0) continue;
        bool is_dup = false;
        for (int re = 0; re < ep_cnt; re++) {
          if (memcmp(ep_arr[re].ip, ep_ip, 16) == 0 && ep_arr[re].port == ep_port) {
            is_dup = true;
            break;
          }
        }
      ...
      for (int re = 0; re < ep_cnt; re++) {
        if (bc >= BATCH_MAX) {
          ...
          if (tx_rc < 0) {
            fprintf(stderr, "udp: batch send failed before enqueue multicast\n");
          }
          bc = 0;
          if (tx_rc < fl_bc || udp_w_want(udp)) break;
        }
    ...
    if (!rt_key_get(rt, frame_pkt, cfg->addr, rt_key)) {
      if (now - l_nh_ts >= 2000ULL) {
        fprintf(stderr, "routing: unresolved next-hop for dst-mac %02x:%02x:%02x:%02x:%02x:%02x\n",
                pl_ptr[0], pl_ptr[1], pl_ptr[2], pl_ptr[3], pl_ptr[4], pl_ptr[5]);
        l_nh_ts = now;
      }
      continue;
    }
    ...
    int fp_idx = (int)((rt_key[12] ^ rt_key[13] ^ rt_key[14] ^ rt_key[15]) & 0xff);
    if (fast_path[fp_idx].is_val && now <= fast_path[fp_idx].val_ts && memcmp(fast_path[fp_idx].dest_lla, rt_key, 16) == 0) {
    ...
    int32_t frag_cap = (int32_t)pmtu - (int32_t)(tnl_oh + (uint16_t)sizeof(FragHdr) + ((rel_f != 0) ? 4U : 0U));
    if (frag_cap <= 0) {
      static uint64_t l_cr_ts = 0;
      if (now >= l_cr_ts + 1000ULL) {
        fprintf(stderr, "main: physical mtu %u too small for tunnel overhead %u\n",
                (unsigned)pmtu, (unsigned)(tnl_oh + (uint16_t)sizeof(FragHdr) + ((rel_f != 0) ? 4U : 0U)));
        l_cr_ts = now;
      }
      continue;
    }
    uint16_t max_tap_f = (pmtu > tnl_oh) ? (uint16_t)(pmtu - tnl_oh) : 0;
    {
      ...
      if ((eth_type == 0x8100U || eth_type == 0x88A8U) && frame_len >= ETH_HLEN + 4 + 20) {
        eth_type = (uint16_t)(((uint16_t)frame_pkt[16] << 8) | frame_pkt[17]);
        l3_off = ETH_HLEN + 4;
      }
      if ((eth_type == 0x0800U || eth_type == 0x86DDU) && frame_len > l3_off) {
        uint16_t max_in_l3 = (max_tap_f > l3_off + 20U) ? (uint16_t)(max_tap_f - l3_off - 20U) : 0;
        if (max_in_l3 >= 88U) {
    ...
    bool is_tiny = false;
    {
      ...
      if ((eth_type == 0x0800U || eth_type == 0x86DDU) && frame_len > l3_off) {
        is_tiny = (frag_cap < 64) && is_tcp_syn(frame_pkt + l3_off, frame_len - l3_off);
      }
    }
    if (!is_tiny && __builtin_expect(((size_t)vnet_len + (size_t)tnl_oh) <= (size_t)pmtu, 1)) {
      ...
      if (bc >= BATCH_MAX) {
        ...
        int tx_rc = udp_tx_arr(udp, batch_arr, bc);
        if (tx_rc < 0) {
          fprintf(stderr, "udp: batch send failed before enqueue data\n");
        }
        bc = 0;
        if (tx_rc < fl_bc || udp_w_want(udp)) break;
      }
      ...
      while (off < vnet_len) {
        ...
        if (bc + need > BATCH_MAX) {
          ...
          int tx_rc = udp_tx_arr(udp, batch_arr, bc);
          if (tx_rc < 0) {
            fprintf(stderr, "udp: batch send failed during frag flush\n");
          }
          bc = 0;
          if (tx_rc < fl_bc || udp_w_want(udp)) break;
        }
        ...
        uint8_t *chunk_dst1 = frag_bufs[bc] + TAP_HR + PKT_HDR_SZ + sizeof(FragHdr) + ((rel_f != 0) ? 4U : 0U);
        memcpy(chunk_dst1, vnet_frame + off, chunk_len);
        if (off > 0x7fffU) break;
        size_t out_len = 0;
        bool mf = (off + chunk_len) < vnet_len;
        uint8_t *out_ptr = frag_bld_zc(cry_ctx, chunk_dst1, chunk_len, msg_id, (uint16_t)off, mf, rel_f, (rel_f != 0) ? rel_dst_tail : NULL, 32, &out_len);
        ...
        bc++;
        if (is_dup_tx) {
          uint8_t *chunk_dst2 = frag_bufs[bc] + TAP_HR + PKT_HDR_SZ + sizeof(FragHdr) + ((rel_f != 0) ? 4U : 0U);
          memcpy(chunk_dst2, vnet_frame + off, chunk_len);
          size_t out_len2 = 0;
          uint8_t *out_ptr2 = frag_bld_zc(cry_ctx, chunk_dst2, chunk_len, msg_id, (uint16_t)off, mf, rel_f, (rel_f != 0) ? rel_dst_tail : NULL, 32, &out_len2);
    ...
    if (bc >= BATCH_MAX - 2) {
      ...
      int tx_rc = udp_tx_arr(udp, batch_arr, bc);
      if (tx_rc < 0) {
        fprintf(stderr, "udp: batch send failed during near-full flush\n");
      }
      bc = 0;
      if (tx_rc < fl_bc || udp_w_want(udp)) break;
    }
  ...
  if (bc > 0) {
    ...
    if (tx_rc < fl_bc || udp_w_want(udp)) return;
  }
}

void on_udp(int tap_fd, Udp *udp, Cry *cry_ctx, Rt *rt, const Cfg *cfg, uint64_t sid, PPool *pool) {
  ...
  for (int i = 0; i < rc; i++) {
    ...
    if (raw_len < PKT_HDR_SZ) continue;
    PktHdr hdr;
    static uint8_t pt_store[TAP_HR + ETH_HLEN + UDP_PL_MAX] __attribute__((aligned(32)));
    uint8_t *pt_buf = pt_store + TAP_HR + ETH_HLEN;
    uint8_t *pt;
    size_t pt_len;
    int dec_res = pkt_dec(cry_ctx, raw_buf, raw_len, pt_buf, sizeof(pt_store) - TAP_HR - ETH_HLEN, &hdr, &pt, &pt_len);
    if (dec_res != 0) continue;
    if (!rx_rp_chk(src_ip, src_port, raw_buf + PKT_CH_SZ)) continue;
    rt_rx_ack(rt, src_ip, src_port, ts);
    switch (hdr.pkt_type) {
    case PT_DATA: {
      g_rx_ts = ts;
      if (hdr.rel_f != 0) {
        if (pt_len < VNET_HL + ETH_HLEN) break;
        if (hdr.hop_c <= 1) {
          fprintf(stderr, "routing: drop: ttl expired, routing loop detected\n");
          break;
        }
        const uint8_t *vnet_frame = pt;
        size_t vnet_len = pt_len;
        if (vnet_len < VNET_HL + ETH_HLEN) break;
        const uint8_t *fwd_frame = vnet_frame + VNET_HL;
        uint8_t dest_lla[16] = { 0 };
        if (!mesh_dst_lla_from_frame(fwd_frame, cfg->addr, dest_lla)) break;
        if (memcmp(dest_lla, cfg->addr, 16) == 0) {
          gro_fed(tap_fd, vnet_frame, vnet_len);
        } else {
          rel_fwd_dat(udp, cry_ctx, rt, cfg, dest_lla, vnet_frame, vnet_len, (uint8_t)(hdr.hop_c - 1), ts, src_ip, src_port);
        }
      } else {
        const uint8_t *vnet_frame = pt;
        size_t vnet_len = pt_len;
    ...
    case PT_FRAG: {
      ...
      uint8_t dest_lla[16] = { 0 };
      if (hdr.rel_f != 0) {
        if (payload_len < 4 + sizeof(FragHdr)) break;
        if (hdr.hop_c <= 1) break;
        memcpy(dest_lla, cfg->addr, 16);
        memcpy(dest_lla + 12, payload, 4);
        payload += 4;
        payload_len -= 4;
      }
      if (payload_len < sizeof(FragHdr)) break;
      uint32_t msg_id = ((uint32_t)payload[0] << 24) | ((uint32_t)payload[1] << 16) | ((uint32_t)payload[2] << 8) | (uint32_t)payload[3];
      uint16_t off_mf = (uint16_t)(((uint16_t)payload[4] << 8) | payload[5]);
      ...
      if (hdr.rel_f != 0 && memcmp(dest_lla, cfg->addr, 16) != 0) {
        relay_fwd_frag(udp, cry_ctx, rt, cfg, dest_lla, msg_id, frag_off, mf, chunk, chunk_len, (uint8_t)(hdr.hop_c - 1), ts, src_ip, src_port);
        break;
      }
      uint16_t full_len = 0;
      uint8_t *full_l3 = frag_asm(msg_id, frag_off, mf, chunk, chunk_len, &full_len);
      if (!full_l3) break;
      if (full_len < VNET_HL + ETH_HLEN) break;
    ...
    case PT_MTU_PRB: {
      ...
      if (gsp_prs_mtu_prb(pt, pt_len, &probe_id, &probe_mtu) != 0) break;
    ...
    case PT_MTU_ACK: {
      uint32_t probe_id = 0;
      uint16_t probe_mtu = 0;
      if (gsp_prs_mtu_ack(pt, pt_len, &probe_id, &probe_mtu) != 0) break;
    ...
    case PT_PING: {
      ...
      if (on_ping(pt, pt_len, &req_ts, &peer_sid, peer_lla) == 0) {
        if (rt_peer_sess(rt, peer_lla, peer_sid, ts)) rx_rp_rst_lla(rt, peer_lla);
        if (hdr.rel_f == 0) rt_ep_upd(rt, peer_lla, src_ip, src_port, ts);
        if (hdr.rel_f == 0 && !is_ip_bgn(src_ip) && !p_is_me(rt, cfg->addr, src_ip, src_port)) {
          pp_add(pool, src_ip, src_port);
        }
        static uint8_t pong_buf[UDP_PL_MAX];
        size_t pong_len;
        pong_bld(cry_ctx, cfg->addr, req_ts, sid, pong_buf, &pong_len);
    ...
    case PT_GSP: {
      if (cfg->p2p == P2P_EN) {
        ...
        on_gsp(pt, pt_len, src_ip, src_port, cfg->addr, rt, pool, ts, &is_mod, &req_seq, seq_tgt);
        if (req_seq) {
...
void on_tmr(int timer_fd, Udp *udp, Cry *cry_ctx, Rt *rt, const Cfg *cfg, int *gsp_off, const char *peers_path, uint16_t act_port, uint64_t sid, PPool *pool) {
  ...
  size_t gsp_len = 0;
  if (is_gsp) {
    gsp_bld(cry_ctx, rt->re_arr, (int)rt->cnt, *gsp_off, cfg->addr, g_buf, &gsp_len);
  }
  bool has_dt = false;
  uint8_t dt_lla[16] = { 0 };
  ...
  if (cfg->p2p == P2P_EN && rt_trg_pop(rt, dt_lla)) {
    gsp_dt_bld(cry_ctx, rt->re_arr, (int)rt->cnt, dt_lla, cfg->addr, dt_buf, &dt_len);
    has_dt = (dt_len > 0);
  }
  static UdpMsg batch_arr[BATCH_MAX];
  int bc = 0;
  for (uint32_t i = 0; i < rt->cnt; i++) {
    if (!rt->re_arr[i].is_act && !rt->re_arr[i].is_static && rt->re_arr[i].state != RT_PND) continue;
    if (rt->re_arr[i].r2d != 0) continue;
    if (is_idle) {
  ...
  if (bc > 0) udp_tx_arr(udp, batch_arr, bc);
  {
    for (int burst = 0; burst < 3; burst++) {
      Re prb_re;
      uint16_t prb_mtu = 0;
      uint32_t probe_id = 0;
      if (!rt_mprb_rdy(rt, ts, &prb_re, &prb_mtu, &probe_id)) break;
      static uint8_t prb_buf[UDP_PL_MAX];
      size_t probe_len = 0;
      uint16_t prb_o_oh = is_ip_v4m(prb_re.ep_ip) ? 20U : 40U;
      uint16_t prb_u_oh = (uint16_t)(prb_o_oh + 8U);
      size_t t_pl_len = (prb_mtu > prb_u_oh) ? (size_t)(prb_mtu - prb_u_oh) : (size_t)PKT_HDR_SZ;
      mtu_prb_bld(cry_ctx, probe_id, prb_mtu, t_pl_len, prb_buf, &probe_len);
      udp_tx(udp, prb_re.ep_ip, prb_re.ep_port, prb_buf, probe_len);
      {
        ...
        udp_tx(udp, prb_re.ep_ip, prb_re.ep_port, prb_ping_buf, prb_ping_len);
      }
  ...
  if (cfg->p2p == P2P_EN) {
    ...
    for (uint32_t i = 0; i < rt->cnt; i++) {
      Re *dec = &rt->re_arr[i];
      if (dec->r2d != 0) continue;
      if (dec->state != RT_PND) continue;
      if (memcmp(dec->lla, z_lla, 16) == 0) continue;
      RtDec rd = rt_sel(rt, dec->lla, true);
      if (rd.type != RT_REL) continue;
      static uint8_t hp_buf[UDP_PL_MAX];
      size_t hp_len = 0;
...
void on_std(int fd, Rt *rt, const Cfg *cfg, PPool *pool) {
  ...
  if (r_len <= 0) return;
  cmd_buf[r_len] = '\0';
  if (cmd_buf[0] == '\n' || cmd_buf[0] == '\r' || cmd_buf[0] == '\x1b') return;
  if (!(strchr(cmd_buf, 's') || strchr(cmd_buf, 'S'))) return;

  int um_cnt = 0;
  uint8_t z_lla[16] = { 0 };

  typedef struct { uint8_t ip[16]; uint16_t port; } UnmappedEp;
  UnmappedEp um_arr[256];

  for (uint32_t re_idx = 0; re_idx < rt->cnt; re_idx++) {
    Re *z_re = &rt->re_arr[re_idx];
    if (z_re->r2d != 0 || memcmp(z_re->lla, z_lla, 16) != 0 || z_re->state == RT_DED) continue;
    bool is_mapped = false;
    for (uint32_t map_idx = 0; map_idx < rt->cnt; map_idx++) {
      Re *m_re = &rt->re_arr[map_idx];
      if (m_re->r2d != 0 || memcmp(m_re->lla, z_lla, 16) == 0 || m_re->state == RT_DED) continue;
      if (memcmp(m_re->ep_ip, z_re->ep_ip, 16) == 0 && m_re->ep_port == z_re->ep_port) {
    ...
    for (int um_idx = 0; um_idx < um_cnt; um_idx++) {
      if (memcmp(um_arr[um_idx].ip, z_re->ep_ip, 16) == 0 && um_arr[um_idx].port == z_re->ep_port) {
        is_dup = true; break;
      }
    }
  ...
  typedef struct { char dst[INET6_ADDRSTRLEN + 2]; char nh[128]; char rtt[32]; char st[10]; char mtu[64]; } RRow;
  uint8_t uniq_lla[256][16];
  int u_cnt = 0;
  for (uint32_t j = 0; j < rt->cnt && u_cnt < 256; j++) {
    if (rt->re_arr[j].state == RT_DED || memcmp(rt->re_arr[j].lla, z_lla, 16) == 0 || memcmp(rt->re_arr[j].lla, cfg->addr, 16) == 0) continue;
    bool is_dup = false;
    for (int u = 0; u < u_cnt; u++) {
      if (memcmp(uniq_lla[u], rt->re_arr[j].lla, 16) == 0) { is_dup = true; break; }
    }
    if (!is_dup) memcpy(uniq_lla[u_cnt++], rt->re_arr[j].lla, 16);
  }
  ...
  for (int u = 0; u < u_cnt && r_cnt < 256; u++) {
    ...
    if ((int)strlen(row->dst) > m_dst) m_dst = (int)strlen(row->dst);
    uint8_t sel_ip[16]; uint16_t sel_port = 0;
    if (sel.type == RT_DIR) { memcpy(sel_ip, sel.dir.ip, 16); sel_port = sel.dir.port; }
    else if (sel.type == RT_REL) { memcpy(sel_ip, sel.rel.relay_ip, 16); sel_port = sel.rel.relay_port; }
    else { memset(sel_ip, 0, 16); sel_port = 0; }

    char ep_str[64] = "-";
    if (sel.type == RT_DIR || sel.type == RT_REL) {
      char ip_str[INET6_ADDRSTRLEN];
      if (sel_ip[0] == 0 && sel_ip[1] == 0 && sel_ip[10] == 0xff && sel_ip[11] == 0xff)
    ...
    if (sel.type == RT_DIR) inet_ntop(AF_INET6, uniq_lla[u], nhop_str, sizeof(nhop_str));
    else if (sel.type == RT_REL && IS_LLA_VAL(sel.rel.relay_lla)) inet_ntop(AF_INET6, sel.rel.relay_lla, nhop_str, sizeof(nhop_str));

    uint32_t show_m = rt_e2e_m(rt, uniq_lla[u], &sel);
    if (sel.type == RT_NONE || sel.type == RT_VP) {
      memset(sel_ip, 0, 16); sel_port = 0;
      snprintf(row->nh, sizeof(row->nh), "- (-)");
      strcpy(row->st, sel.type == RT_NONE ? "unmapped" : "via");
      strcpy(row->rtt, "-"); strcpy(row->mtu, "-");
    } else {
      snprintf(row->nh, sizeof(row->nh), "%s (%s)", nhop_str, ep_str);
      if (show_m >= RT_M_INF) strcpy(row->rtt, "-");
      else snprintf(row->rtt, sizeof(row->rtt), "%ums", show_m);
      ...
      if (is_srch) {
        if (prb_mtu > 0) snprintf(row->mtu, sizeof(row->mtu), "%u (%u-%u, %u)", pmtu, lkg, ukb, prb_mtu);
        else snprintf(row->mtu, sizeof(row->mtu), "%u (%u-%u)", pmtu, lkg, ukb);
      } else {
        if (mtu_st == MTU_ST_F || is_fixed) snprintf(row->mtu, sizeof(row->mtu), "%u", pmtu);
        else snprintf(row->mtu, sizeof(row->mtu), "%u (LKG:%u, UKB:%u, base)", pmtu, lkg, ukb);
      }
      strcpy(row->st, show_m >= RT_M_INF ? "pending" : "active");
    }
    if ((int)strlen(row->nh) > m_nh) m_nh = (int)strlen(row->nh);
    if (sel.type != RT_NONE && sel.type != RT_VP && show_m >= RT_M_INF) strcpy(row->st, "pending");
    if (strcmp(row->st, "active") == 0) act_map_cnt++;
    if ((int)strlen(row->st) > m_st) m_st = (int)strlen(row->st);
    if ((int)strlen(row->mtu) > m_mtu) m_mtu = (int)strlen(row->mtu);
  }
  ...
  for (int um_idx = 0; um_idx < um_cnt && r_cnt < 256; um_idx++) {
    ...
    if (ep_ip[0] == 0 && ep_ip[1] == 0 && ep_ip[10] == 0xff && ep_ip[11] == 0xff) inet_ntop(AF_INET, ep_ip + 12, ip_str, sizeof(ip_str));
    else inet_ntop(AF_INET6, ep_ip, ip_str, sizeof(ip_str));
    snprintf(row->nh, sizeof(row->nh), "- (%s:%u)", ip_str, um_arr[um_idx].port);
    strcpy(row->st, "unmapped"); strcpy(row->rtt, "-"); strcpy(row->mtu, "-");
    if ((int)strlen(row->dst) > m_dst) m_dst = (int)strlen(row->dst);
    if ((int)strlen(row->nh) > m_nh) m_nh = (int)strlen(row->nh);
    if ((int)strlen(row->st) > m_st) m_st = (int)strlen(row->st);
    if ((int)strlen(row->mtu) > m_mtu) m_mtu = (int)strlen(row->mtu);
  ...
  if (r_cnt > 0) {
    printf("  %-*s  %-*s  %-*s  %-*s  rtt\n", m_dst, "dst", m_nh, "nexthop", m_st, "state", m_mtu, "mtu");
    for (int k = 0; k < r_cnt; k++) {
      printf("  %-*s  %-*s  %-*s  %-*s  %s\n", m_dst, rows[k].dst, m_nh, rows[k].nh, m_st, rows[k].st, m_mtu, rows[k].mtu, rows[k].rtt);
    }
  }
}