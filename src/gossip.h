...
uint8_t *gsp_bld (Cry *s, Rt *rt, int s_off,
                  ...
                  bool dst_no_dir, const uint8_t dst_lla[16], uint8_t *buf,
                  size_t *out_len);
...
uint8_t *gsp_dt_bld (Cry *s, Rt *rt,
                     ...
                     bool self_dir_ok, bool dst_no_dir,
                     const uint8_t dst_lla[16], uint8_t *buf,
                     size_t *out_len);
...
uint8_t *data_bld_zc_cnt (Cry *s, uint8_t *ipv6_ptr_start,
                          ...
                          uint64_t cnt, const uint8_t dst_lla[16],
                          size_t *out_len);
uint8_t *data_bld_zc (Cry *s, uint8_t *ipv6_ptr_start, size_t ipv6_len,
                      uint8_t rel_f, uint8_t hop_c,
...
uint8_t *mtu_prb_bld (Cry *s, uint32_t probe_id, uint16_t probe_mtu,
                      size_t t_pl_len, const uint8_t dst_lla[16],
                      uint8_t *buf, size_t *out_len);
uint8_t *mtu_ack_bld (Cry *s, uint32_t probe_id, uint16_t probe_mtu,
                      const uint8_t dst_lla[16], uint8_t *buf,
                      size_t *out_len);
uint8_t *stat_req_bld (Cry *s, uint32_t req_id, const uint8_t dst_lla[16],
                       uint8_t *buf, size_t *out_len);
...
int pkt_dec (Cry *s, uint8_t *raw, size_t raw_len, uint8_t *pt_buf,
             ...
             size_t *pt_len_out);
int pkt_dec_from (Cry *s, const uint8_t src_lla[16], uint8_t *raw,
                  size_t raw_len, uint8_t *pt_buf, size_t pt_len,
                  PktHdr *hdr_out, uint8_t **pt_out, size_t *pt_len_out,
...
int on_pong (const uint8_t *pt, size_t pt_len, uint64_t *o_ts, uint64_t *sid,
             ...
             uint64_t *prb_tok);
const uint8_t *on_ping_kx (const uint8_t *pt, size_t pt_len);
const uint8_t *on_pong_kx (const uint8_t *pt, size_t pt_len);
int on_gsp (const uint8_t *pt, size_t pt_len, const uint8_t src_ip[16],
            uint16_t src_port, const uint8_t src_lla_hint[16],
            const uint8_t our_lla[16], Rt *rt, PPool *pool,
            bool allow_dir_hint, uint64_t sys_ts, bool *is_mod,
            bool *req_seq, uint8_t seq_tgt[16]);
